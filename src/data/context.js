(function(root,factory){
  ...
  function buildContext(data={},view='dashboard',activeMissionIndex=-1){
    ...
    if(view==='travel')ctx.trips=take(data.trips,10).map(x=>({destination:clip(x.destination),start:x.start||'',end:x.end||'',budget:Number(x.budget)||0,missionId:x.missionId||'',packing:take(x.packing,12).map(p=>({text:clip(p.text,120),done:!!p.done})),itinerary:take(x.itinerary,12).map(it=>({date:it.date||'',text:clip(it.text,160)}))}));
